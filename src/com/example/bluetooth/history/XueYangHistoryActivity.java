...
public class XueYangHistoryActivity extends AbActivity {
	
	
	  private  List<HistoryXueYang>  getData() {
			FinalDb db = FinalDb.create(XueYangHistoryActivity.this);
			 List<HistoryXueYang> history = db.findAll(HistoryXueYang.class);
			if(history.size() > 0){
	  ...
	  public void clear_notes(View v){
			//拿到FinalDb对象引用
		    FinalDb db = FinalDb.create(XueYangHistoryActivity.this);
		    db.deleteAll(HistoryXueYang.class);//删除Bean对应的数据表中的所有数据
		    Dialog dialog = new AlertDialog.Builder(this).setIcon(
		    	     android.R.drawable.btn_star).setTitle("删除成功").setMessage(
		    	     "已清除血痒数据").setPositiveButton("确定",new OnClickListener() {
		    	      @Override
	  ...
	  private void refresh() {  
	      finish();  
	      Intent intent = new Intent(XueYangHistoryActivity.this, XueYangHistoryActivity.class);  
    ...
    public void onCreate(Bundle savedInstanceState) {
	    ...
	    List<HistoryXueYang> history = getData();
	    ...
	    //临界线
	    double[] limit = new double[]{100,94};
	    renderer.setmYLimitsLine(limit);
	    int[] colorsLimit = new int[] { Color.rgb(0, 0,255), Color.rgb(0, 0,255) };
