...
public class TiwenHistoryActivity extends AbActivity {
	
	
	  private  List<HistoryTiWen>  getData() {
			FinalDb db = FinalDb.create(TiwenHistoryActivity.this);
			 List<HistoryTiWen> history = db.findAll(HistoryTiWen.class);
			if(history.size() > 0){
	  ...
	  public void clear_notes(View v){
			//拿到FinalDb对象引用
		    FinalDb db = FinalDb.create(TiwenHistoryActivity.this);
		    db.deleteAll(HistoryTiWen.class);//删除Bean对应的数据表中的所有数据
		    Dialog dialog = new AlertDialog.Builder(this).setIcon(
		    	     android.R.drawable.btn_star).setTitle("删除成功").setMessage(
		    	     "已清除体温数据").setPositiveButton("确定",new OnClickListener() {
	  ...
	  private void refresh() {  
	      finish();  
	      Intent intent = new Intent(TiwenHistoryActivity.this, TiwenHistoryActivity.class);  
    ...
    public void onCreate(Bundle savedInstanceState) {
	    ...
	    List<HistoryTiWen> history = getData();
